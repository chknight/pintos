...
void print_n() {
  ...
  struct list_elem *p;
  for (p = list_begin(&number_list); p != list_end(&number_list); p = list_next(p)) {
    struct sortNumber *temp = list_entry(p, struct sortNumber, elem);
...
void produce_n() {
  lock_acquire(&number_lock);
  struct sortNumber *temp = NULL;
  int i;
  printf("The produced number are\n");
  for (i = 0; i < 20; ++i) {
    temp = (struct sortNumber *)malloc(sizeof(struct sortNumber));
    random_bytes(&temp->number, sizeof(int));
    printf("%d ", temp->number);
    list_push_back(&number_list, &(temp->elem));
    temp = NULL;
  }
  printf("\n\n");
  lock_release(&number_lock);
...
void sort_n() {
  lock_acquire(&number_lock);
  list_sort(&number_list, compare, NULL);
...
hello_pintos (void) {
  ...
  printf("hello, cuihang\n");
  thread_create("produce_n", 63, produce_n, NULL);
  thread_create("sort_n", 63, sort_n, NULL);
  thread_create("print_n", 63, print_n, NULL);
  lock_acquire(&number_lock);
  lock_release(&number_lock);
  //test your code here
}
